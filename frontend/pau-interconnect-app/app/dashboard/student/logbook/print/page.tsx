...
export default function LogbookPrintView() {
    ...
    const fetchAll = async () => {
      try {
        ...
        const { data: apps } = await supabase
          ...
          .eq("user_id", user.id);
        const accepted = apps?.find((a: any) => {
          const s = a.status?.toLowerCase();
          return s === "accepted" || s === "approved";
        });
        if (accepted) setInternship(accepted.internship);
        
        const logRes: any = await authenticatedFetch(`/api/logbook/student?student_id=${user.id}`, {}, 10000);
        const sorted = (logRes.entries || []).sort((a: any, b: any) => new Date(a.date).getTime() - new Date(b.date).getTime());
        setEntries(sorted);
      } catch (err) {
  ...
  const groupEntriesByWeek = () => {
    ...
    const weeks: { weekNumber: number; startDate: string; endDate: string; days: LogbookEntry[] }[] = [];
    ...
    entries.forEach(entry => {
      ...
      if (!week) {
        ...
        week = { 
          ...
          endDate: wEnd.toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }), 
          days: [] 
        };
        weeks.push(week);
      }
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-white">
        <p className="text-sm font-medium text-slate-500 uppercase">Loading Logbook...</p>
      </div>
    );
  }
  ...
  return (
    <div className="bg-white text-black min-h-screen p-8 max-w-4xl mx-auto font-serif print:p-0 print:m-0">
      <div className="text-center mb-10 border-b-2 border-black pb-6">
        <h1 className="text-3xl font-black uppercase tracking-widest mb-2">Students Industrial Work Experience Scheme</h1>
        <h2 className="text-xl font-bold tracking-widest mb-6">OFFICIAL SIWES LOGBOOK</h2>
        
        <div className="grid grid-cols-2 gap-4 text-left text-sm mt-8 border-2 border-black p-6">
          <div className="space-y-1">
            <p><strong>NAME:</strong> {profile?.full_name?.toUpperCase()}</p>
            <p><strong>INSTITUTION:</strong> PAN-ATLANTIC UNIVERSITY</p>
            <p><strong>DEPARTMENT:</strong> {profile?.department || 'N/A'}</p>
          </div>
          <div className="space-y-1 border-l-2 border-black pl-6">
            {internship && (
              <>
                <p><strong>ORGANIZATION:</strong> {internship.company?.toUpperCase()}</p>
                <p><strong>ROLE:</strong> {internship.role?.toUpperCase()}</p>
              </>
            )}
            <p><strong>TOTAL ENTRIES:</strong> {entries.length} Days</p>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-12">
        {weeklyData.length === 0 ? (
          <p className="text-center italic mt-20">No logbook entries available for this period.</p>
        ) : (
          weeklyData.map((week) => (
            <div key={week.weekNumber} className="break-inside-avoid">
              <div className="flex items-center gap-4 mb-4">
                <h3 className="text-lg font-black uppercase bg-black text-white px-3 py-1">Week {week.weekNumber}</h3>
                <span className="text-sm font-bold border-b-2 border-black flex-1 pb-1">{week.startDate} — {week.endDate}</span>
              </div>

              <div className="border-2 border-black">
                {week.days.map((day, idx) => (
                  <div key={day.id} className={idx !== 0 ? "border-t-2 border-black" : ""}>
                    <div className="flex border-b border-black bg-slate-50">
                      <div className="w-40 border-r border-black p-2 font-bold text-xs uppercase shrink-0">
                        ...
                        <br />
                        <span className="font-normal opacity-60 font-sans">{day.date}</span>
                      </div>
              ...
              <div className="mt-4 grid grid-cols-2 gap-4 border-2 border-black border-t-0 p-4 bg-slate-50/50">
                <div className="text-[11px] space-y-4">
                  <p className="font-bold border-b border-black pb-1">INDUSTRY SUPERVISOR'S COMMENTS</p>
                  <div className="h-10"></div>
                  <p className="border-t border-dotted border-black pt-1">NAME & SIGNATURE / DATE</p>
                </div>
                <div className="text-[11px] space-y-4 border-l-2 border-black pl-4">
                  <p className="font-bold border-b border-black pb-1">INSTITUTION SUPERVISOR'S COMMENTS</p>
                  <div className="h-10"></div>
                  <p className="border-t border-dotted border-black pt-1">NAME & SIGNATURE / DATE</p>
                </div>
              </div>
