...
export default async function ManagePlatformView() {
  ...
  try {
    profiles =
      await authenticatedFetchServer<DirectoryProfile[]>("/admin/directory");
  } catch (err: unknown) {
  ...
  const employers = (profiles || []).filter(
    (p) => !!p && p.role === "employer",
  );
  const students = (profiles || []).filter(
    (p) => !!p && (!p.role || p.role === "student" || p.role === "admin"),
  );
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="space-y-1">
          ...
          <Typography variant="body1" color="muted">
            Manage all registered users and companies
          </Typography>
        </div>
